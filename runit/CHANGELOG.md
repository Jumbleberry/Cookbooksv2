...
- Donât mask errors from shellout helper if the binary is present.
...
- Update wording to clarify that weâre deleting not âzappingâ files
- Donât hang forever if Runit isnât installed when using the provider
- Check for the runit binary before every shellout
- Remove Fedora support since it doesnât work
